...
def _load_weights(get_tensor: Callable[[str], torch.Tensor], model: nn.Module) -> None:
    ...
    model.vision["patch_emb"].weight.data.copy_(
        get_tensor("model.vision.patch_emb.weight")
    )
    model.vision["patch_emb"].bias.data.copy_(
        get_tensor("model.vision.patch_emb.bias")
    )
    model.vision.pos_emb.data.copy_(
        get_tensor("model.vision.pos_emb")
    )
    ...
    model.vision["post_ln"].weight.data.copy_(
        get_tensor("model.vision.post_ln.weight")
    )
    model.vision["post_ln"].bias.data.copy_(
        get_tensor("model.vision.post_ln.bias")
    )

    model.vision["proj_mlp"]["fc1"].weight.data.copy_(
        get_tensor("model.vision.proj_mlp.fc1.weight")
    )
    model.vision["proj_mlp"]["fc1"].bias.data.copy_(
        get_tensor("model.vision.proj_mlp.fc1.bias")
    )
    model.vision["proj_mlp"]["fc2"].weight.data.copy_(
        get_tensor("model.vision.proj_mlp.fc2.weight")
    )
    model.vision["proj_mlp"]["fc2"].bias.data.copy_(
        get_tensor("model.vision.proj_mlp.fc2.bias")
    )
    ...
    for i in range(len(model.text["blocks"])):
        ...
        model.text["blocks"][i]["ln"].bias.data.copy_(
            get_tensor(f"{prefix}.ln.bias")
        )
    ...
    model.text["post_ln"].weight.data.copy_(
        get_tensor("model.text.post_ln.weight")
    )
    model.text["post_ln"].bias.data.copy_(
        get_tensor("model.text.post_ln.bias")
    )

    model.text["lm_head"].weight.data.copy_(
        get_tensor("model.text.lm_head.weight")
    )
    model.text["lm_head"].bias.data.copy_(
        get_tensor("model.text.lm_head.bias")
    )

    # Region Model
    model.region.coord_features.data.copy_(
        get_tensor("model.region.coord_features")
    )
    model.region["coord_encoder"].weight.data.copy_(
        get_tensor("model.region.coord_encoder.weight")
    )
    model.region["coord_encoder"].bias.data.copy_(
        get_tensor("model.region.coord_encoder.bias")
    )

    model.region["coord_decoder"]["fc1"].weight.data.copy_(
        get_tensor("model.region.coord_decoder.fc1.weight")
    )
    model.region["coord_decoder"]["fc1"].bias.data.copy_(
        get_tensor("model.region.coord_decoder.fc1.bias")
    )
    model.region["coord_decoder"]["fc2"].weight.data.copy_(
        get_tensor("model.region.coord_decoder.fc2.weight")
    )
    model.region["coord_decoder"]["fc2"].bias.data.copy_(
        get_tensor("model.region.coord_decoder.fc2.bias")
    )

    model.region.size_features.data.copy_(
        get_tensor("model.region.size_features")
    )
    model.region["size_encoder"].weight.data.copy_(
        get_tensor("model.region.size_encoder.weight")
    )
    model.region["size_encoder"].bias.data.copy_(
        get_tensor("model.region.size_encoder.bias")
    )

    model.region["size_decoder"]["fc1"].weight.data.copy_(
        get_tensor("model.region.size_decoder.fc1.weight")
    )
    model.region["size_decoder"]["fc1"].bias.data.copy_(
        get_tensor("model.region.size_decoder.fc1.bias")
    )
    model.region["size_decoder"]["fc2"].weight.data.copy_(
        get_tensor("model.region.size_decoder.fc2.weight")
    )
    model.region["size_decoder"]["fc2"].bias.data.copy_(
        get_tensor("model.region.size_decoder.fc2.bias")
    )
...
def load_weights_from_safetensors(weights_file: str, model: nn.Module) -> None:
    ...
    with safetensors_open(weights_file) as get_tensor:
        _load_weights(get_tensor, model)
